...
// world portion.
//
// As of 0.2.0 the deserialization pipeline is:
...
export class DocumentSerializer extends Serializer {
    ...
    validate(json) {
        ...
        // Protocol version check (domain-level, not envelope-level).
        const errors = [];
        if (json.metadata && json.metadata.protocolVersion !== PROTOCOL_VERSION) {
            errors.push(
                `Unsupported protocol version: document is ${json.metadata.protocolVersion}, `
                + `engine supports ${PROTOCOL_VERSION}`
            );
        }
        // Schema version check (envelope-level).
        if (json.schemaVersion !== undefined && json.schemaVersion !== DOCUMENT_SCHEMA_VERSION) {
            errors.push(
                `Unsupported schema version: document is ${json.schemaVersion}, `
                + `engine supports ${DOCUMENT_SCHEMA_VERSION}`
            );
        }
        ...
        const warnings = [...structural.warnings, ...worldValidation.warnings];
