...
//
// `application/arweave/ArweavePublicationMaterialUploader.js` (0.9.45) is the only
// concrete `materialUploader` (`{ upload(material) -> Promise<uri|null>,
// storage }`) `application/publication/distribution/PublicationDistributionExecutor.js`'s own
...
//        │
//        ▼
//   application/publication/distribution/ContentStorePublicationMaterialUploader.js   ★ (THIS)
//        #upload(material)
//        │
//        ▼
...
// `application/publication/distribution/PublicationDistributionExecutor.js` already expects back
// from `materialUploader.upload()`, the identical narrow translation
// `application/arweave/ArweavePublicationMaterialUploader.js` itself performs for
...
// reads. `application/publication/distribution/PublicationMaterialUploaderComposition.js` is where a
...
//   choice.** `application/publication/distribution/PublicationMaterialUploaderComposition.js`'s own
//   job, one layer up.
...
export class ContentStorePublicationMaterialUploader {
    ...
    // `application/publication/distribution/PublicationDistributionExecutor.js` already reads off
