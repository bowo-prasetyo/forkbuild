...
// In-memory store answering one question:
// the most recently remembered lifecycle description for this publication.
// It holds a single current value per publication identity: never a history,
// never an event stream, never a cache with eviction or expiry. It never
// talks to another process, tab, or machine: persistence is layered on top by
...
//
// Discovery observations are a second, independent map,
// KEYED BY `(publicationId, discoveryProvider)` and additionally by
// discoveryOrigin when one is supplied. Re-observing the same key replaces;
// different providers or origins (for example two Nostr relays) coexist. They never notify, and remove()/clear() drop them with the
// publication.
