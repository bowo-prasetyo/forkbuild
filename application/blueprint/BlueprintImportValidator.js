...
import { validatePublicationAnchor, PublicationAnchorError } from '../anchoring/PublicationAnchorValidator.js';
import {
    validatePublicationSnapshotPlacement,
    PublicationSnapshotPlacementError
} from '../snapshot/placement/PublicationSnapshotPlacementValidator.js';
...
// deliberately kept separate from application/blueprint/ImportBlueprintUseCase.js —
...
// application/blueprint/ImportBlueprintUseCase.js's own execute() always does.
...
// THEN construct" order application/blueprint/ImportBlueprintUseCase.js's own
// header requires.
export function validateBlueprintPackage(pkg, { registry = null } = {}) {
    ...
    // entirely OPTIONAL (see application/blueprint/BlueprintPackage.js's own
    ...
    // above, one concept over — see application/blueprint/BlueprintPackage.js's own
    ...
    // application/blueprint/BlueprintPackage.js's own header. Each entry only ever
    // needs to be a well-formed application/anchoring/PublicationAnchorValidator.js
    ...
    // over — see application/blueprint/BlueprintPackage.js's own header. Each entry
