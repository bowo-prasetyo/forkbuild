...
import { findLiveConnectedPeers, findLiveConnectedDevices } from '../peer/ConnectedIdentityPeers.js';
...
//   PeerRelationship    application/peer/PeerRelationshipUseCase.js   durable
//   FriendshipRecord    application/identity/FriendRelationshipUseCase.js  durable
//   Presence/Connection application/peer/ConnectedPeerRegistry.js      ephemeral
//   Conversation        application/chat/ConversationStore.js          durable
//   Outbox              application/chat/ChatOutbox.js                 durable
//   Read state          application/chat/ConversationReadTracker.js    durable
//
...
// fresh from `application/peer/ConnectedPeerRegistry.js`, exactly the way
...
// collaborator (default: `application/identity/SocialIdentityResolver.js#resolveDirectSocialIdentity`,
...
// currently knows" boundary `application/chat/ChatUseCase.js#_findAuthenticatedPeer()`
...
export class PeerPresenceUseCase {
    constructor({
        ...
        // those already has `application/chat/ChatUseCase.js#onMessage()` to
    ...
    // `application/chat/ChatUseCase.js#_findAuthenticatedPeer()` already
    ...
    // stable order). Mirrors `application/chat/ConversationStore.js#conversations()`'s
    ...
    // to `application/chat/ConversationReadTracker.js#markRead()`, so that
    ...
    // that subscribes to `application/chat/ChatUseCase.js#onMessage()`
    ...
    dispose() {
        ...
        // application/chat/ChatUseCase.js's own dispose().
    }
    ...
    //
    // 0.2.86 — now a thin call into application/peer/ConnectedIdentityPeers.js#findLiveConnectedPeers(),
    // the SAME shared function application/chat/VoiceUseCase.js's own identity-
