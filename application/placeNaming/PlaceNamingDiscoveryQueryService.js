...
//        (any transport: application/nostr/NostrSnapshotDiscoveryQueryService.js's
//        own 0.9.133 shape, a future peer adapter, a future Arweave
//        GraphQL adapter — duck-typed, never an `instanceof` check)
//                    │
//                    ▼
//   application/placeNaming/PlaceNamingDiscoveryQueryService.js   ★ (THIS)
...
// identical restraint `application/snapshot/DiscoverSnapshotCandidatesCommand.js`'s
...
// mirrors `application/placeNaming/LocalPlaceNamingClaimStore.js#has()`'s and
// `application/placeNaming/PlaceNamingClaimExchange.js#importClaim()`'s own already-
...
// restraint `application/snapshot/DiscoverSnapshotCandidatesCommand.js`'s own
// header already holds for a malformed `discoveryQueryService`. An EMPTY
// `sources` array is explicitly valid — see
// `application/placeNaming/PlaceNamingDiscoveryRuntimeComposition.js`'s own header
