...
//
// The second `kindPlugin` for application/publication/PublicationResolver.js, and
// the one that actually PROVES the pipeline is generic rather than
// merely reusable: application/blueprint/BlueprintAttributionPublicationKind.js
...
// envelope module instead (`application/placeNaming/PlaceNamingClaimPublication.js`'s
...
// application/publication/PublicationResolver.js never assumed either shape.
//
// A SECOND transport for a PlaceNamingClaim, never a replacement for
// application/placeNaming/PlaceNamingClaimExchange.js's own 0.5.3 file-exchange
...
// application/placeNaming/PlaceNamingClaimExchange.js#importClaim() never had one
...
// application/publication/PublicationResolver.js's own header.
//
...
// (application/publication/PublicationResolutionCoordinator.js) never imports it
// into application/placeNaming/LocalPlaceNamingClaimStore.js as a side effect of
...
export function createPlaceNamingClaimPublicationKind({ verifier, store = null }) {
    ...
    if (store) {
        ...
        // application/placeNaming/PlaceNamingClaimExchange.js#importClaim() already
