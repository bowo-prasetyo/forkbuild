...
// become durable local content — application/snapshot/materialization/StoreSnapshotContentUseCase.js
...
// boundary. It is deliberately NOT application/peer/PeerContentExchange.js
// (0.7.4) reused or extended — that class already has a job, wired into
// application/publication/PublicationResolutionCoordinator.js (0.7.5/0.7.6) to ask
...
// or silently duplicate application/snapshot/materialization/StoreSnapshotContentUseCase.js's own
...
// already made keeping application/publication/PublicationPeerExchange.js, application/
...
// application/snapshot/materialization/MaterializeSnapshotFromPlacementUseCase.js (a resolved
...
// unlike application/peer/PeerContentExchange.js#request()/_handleRequest(),
// which both refuse to act on a hash absent from this replica's own
// application/publication/LocalPublicationCatalog.js, NEITHER side of this class ever
...
// restraint application/peer/PeerContentExchange.js's own header already
...
export class PublicationSnapshotContentPeerExchange {
    ...
    // straight to application/snapshot/materialization/StoreSnapshotContentUseCase.js, which
    ...
    // event (application/snapshot/materialization/MaterializeSnapshotFromPeerUseCase.js) ignores
    // anything that does not match the one `contentHash` it is currently
    // waiting on, and the only path that can ever write a byte
    // (application/snapshot/materialization/StoreSnapshotContentUseCase.js) always re-verifies
...
// Namespaced separately from application/peer/PeerContentProtocol.js's own
