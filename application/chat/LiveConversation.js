...
// application/chat/ChatUseCase.js's own header, "0.2.61 Ships Live Chat, Not
// A Message Database" (docs/Principles.md): this is in-memory-only
// session state, exactly as durable as the application/chat/ChatUseCase.js
...
// one chat message with THIS SESSION — see application/chat/ChatUseCase.js's
...
// application/chat/ChatUseCase.js, before a message is ever appended here.
//
...
// application/chat/ChatOutbox.js. `updateDeliveryState()` mutates this
...
// application/chat/ChatUseCase.js#_publishDeliveryState(), the only caller.
export class LiveConversation {
    ...
    // this class never re-validates it — see application/chat/ChatUseCase.js,
    ...
    // out-of-order event — see application/chat/ChatUseCase.js's own
