...
// application/chat/ChatUseCase.js's own reconnect-triggered flush reads
...
// shape application/peer/PeerRelationshipUseCase.js already established —
// see that file's own header. `application/chat/ChatUseCase.js`'s own
...
// application/chat/CreateChatOutboxUseCase.js, supplies a genuine one.
//
...
// application/chat/ChatUseCase.js's own reconnect-triggered flush correct
...
// application/chat/LiveConversation.js (0.2.61), which is NEVER durable at
...
export class ChatOutbox {
    ...
    // application/peer/PeerRelationshipUseCase.js#getRelationship's own
    // per-identity filter.
    ...
    // Alice's "Send" gesture when application/chat/ChatUseCase.js#sendOrQueue()
    ...
    // QUEUED -> SENT, once application/chat/ChatUseCase.js has actually
    ...
    // messageId) — application/chat/ChatUseCase.js#_handleIncomingAck() is
    ...
    // application/chat/ChatUseCase.js#_attemptFlush() pass, rather than on
    ...
    // JUST pruned so a caller (application/chat/ChatUseCase.js) can tell a
    ...
    // QUEUED for them — see application/chat/ChatUseCase.js#_cancelOutboxFor(),
