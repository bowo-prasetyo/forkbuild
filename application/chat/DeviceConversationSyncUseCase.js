...
// application/chat/ConversationStore.js/application/chat/ConversationReadTracker.js —
...
// application/chat/ChatUseCase.js's own `forkbuild:chat` protocol, never
...
// this device's own application/chat/ChatUseCase.js accepts or sends
...
// `Sync(S, S) = S` — entirely because application/chat/ChatUseCase.js#ingestSyncedEntry()
// and application/chat/SiblingReadStateStore.js#recordObserved() are each
...
// application/identity/DeviceAuthorizationPropagationUseCase.js's own
...
export class DeviceConversationSyncUseCase {
    ...
    // application/chat/ConversationReadTracker.js#markRead() itself — this
    ...
    // shape application/chat/ChatUseCase.js#sendReadReceipt() already
    ...
    // application/chat/SiblingReadStateStore.js's own header: a purely derived
    // maximum of this device's OWN local read marker
    // (application/chat/ConversationReadTracker.js, per-device, untouched by
    // this milestone) and the highest position any sibling has reported
    // about itself (application/chat/SiblingReadStateStore.js). Recomputed
    // fresh on every call from two independently-read values, never
    // cached — the identical "derived, never a fourth store" discipline
    // application/presence/PeerPresenceUseCase.js's own header already established
    // for presence.
    ...
    // application/chat/ConversationReadTracker.js, exposed here so a caller
    ...
    // for every message this device's own application/chat/ChatUseCase.js
    // just newly learned about via a sibling's sync push (never for a
    // duplicate it already held) — a UI subscribes once, alongside
    // application/chat/ChatUseCase.js#onMessage(), to know a re-render
    ...
    // what application/chat/SiblingReadStateStore.js already held.
    ...
    // the same posture application/chat/ChatUseCase.js#_flushIfAuthenticated()
    ...
    // application/chat/ChatUseCase.js#onMessage()'s own event already fires
    ...
    // application/chat/ChatUseCase.js#ingestSyncedEntry() only republishes
    ...
    // application/chat/ConversationStore.js#append()'s own dedup and stop
