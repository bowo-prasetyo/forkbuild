...
// exceeded, exactly like `application/chat/ChatOutbox.js#pruneExpired`
...
// application/chat/LiveConversation.js's own header (0.2.61) named and
...
// application/chat/ChatOutbox.js and application/peer/PeerRelationshipUseCase.js
...
// Connection — the identical property application/chat/ChatOutbox.js's own
...
// here happens strictly AFTER application/chat/ChatUseCase.js's own
...
// application/chat/LiveConversation.js already had with ChatUseCase, now
...
// from.
//
// Never Reused As, Or By, application/chat/ChatOutbox.js (0.2.69): the
...
export class ConversationStore {
    ...
    // the same acceptance/send order application/chat/LiveConversation.js
    // already displays in. Filters to one peer identity when asked,
    // mirroring application/chat/ChatOutbox.js#list's own per-identity
    // filter.
    ...
    // first. `application/chat/ChatUseCase.js` reads this once, at
    ...
    // application/chat/ChatUseCase.js's own write-through, called for EVERY
    ...
    // see application/chat/ChatUseCase.js's own header) is a harmless no-op
    ...
    // application/chat/ChatUseCase.js's own write-through for a delivery-state
    // transition (QUEUED -> SENT -> DELIVERED, or -> EXPIRED) — mirrors
    // application/chat/LiveConversation.js#updateDeliveryState's own
...
// `application/chat/ChatOutbox.js#pruneExpired` already uses for its own,
