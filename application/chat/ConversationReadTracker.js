...
// `application/chat/ChatOutbox.js` (0.2.63, "what have I sent that hasn't
// been confirmed delivered") and `application/chat/ConversationStore.js`
...
// discipline `application/chat/ConversationStore.js`'s own header names
...
// `application/chat/ConversationStore.js` directly — it is handed a bare
// `peerIdentityId` and the sequence number to advance to by its one
// caller, `application/presence/PeerPresenceUseCase.js#markRead()`, which is the
...
export class ConversationReadTracker {
    ...
    // `application/identity/FriendRelationshipUseCase.js#getState()` already
    ...
    // many peers at once (application/presence/PeerPresenceUseCase.js#list()).
