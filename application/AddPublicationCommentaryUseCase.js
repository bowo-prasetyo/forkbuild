...
// already drew, still intact. This use case answers only "who is Alice,
// really" (derived from the app's own identity infrastructure, never
// from anything a UI could substitute); it still does not ask, let
// alone answer, "is Alice allowed to comment on this Publication."
// Nothing here reads publicationId to make an authorization decision —
// that remains a fully separate, later milestone, exactly as 0.9.244
// deferred it.
//
...
export class AddPublicationCommentaryUseCase {
    constructor(store, identityProvider) {
        if (!store) {
    ...
    // currently-authenticated signing identity, constructs a
    // PublicationCommentary, and persists it through the injected store.
    // `input.authorIdentityId`, if present, is never read — see this
    // file's own header. Returns `{ commentary, isNew }`, identically to
    // 0.9.244. Throws a plain Error when no identity is authenticated
    // (before constructing or persisting anything), a plain Error for
    // structurally invalid input (blank content, missing publicationId —
    // PublicationCommentary's own constructor validation, unchanged),
    // and PublicationCommentaryConflictError when the same commentaryId
    // is already on file with different content. Never catches a
    // storage provider's own write failure.
