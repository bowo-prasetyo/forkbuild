...
// unchanged; repetition (application/editor/RepeatSelectionUseCase.js) is new
// but lives outside this class entirely — it creates bricks, which is
// application/editor/CopySelectionUseCase.js/PasteClipboardUseCase.js territory,
...
export class SpatialEditingService {
    ...
    // (application/editor/EditorSession.js's own construction, and every test
    // that builds this class with 3 or 4 arguments) is therefore
    // completely unaffected; only a caller that explicitly wires a
    // REAL predicate (application/world/WorldNavigationSession.js, backed by
    // application/identity/WorldAuthorizationService.js) gets a gate that can
    // ever say no.
