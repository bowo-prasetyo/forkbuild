...
// own DEFAULT_GATEWAY_URL and application/ArweaveSnapshotDiscoveryPublisher.js's
...
// Deliberately IDENTICAL to application/ArweaveSnapshotDiscoveryPublisher.js's
...
// stay distinct from application/ArweaveGraphqlDiscoveryQueryService.js's
...
//
// The reading counterpart of application/ArweaveSnapshotDiscoveryPublisher.js
...
// live, that the existing `application/ArweaveGraphqlDiscoveryQueryService.js`
...
//        │                                     0.9.133, UNMODIFIED)
//         ▼
//   application/ArweaveSnapshotDiscoveryQueryService.js   ★ (THIS)
//        .search(discoveryTag)
//         │
//         ▼
...
// (application/DecentralizedWorldDiscoveryQuery.js), AND NEVER A SUBCLASS OR
// IN-PLACE GENERALIZATION OF `application/ArweaveGraphqlDiscoveryQueryService.js`.
// The identical restraint `application/NostrSnapshotDiscoveryQueryService.js`'s
// own header already holds against `application/NostrDiscoveryQueryService.js`,
...
//
// SIBLING TO `application/ArweaveSnapshotDiscoveryPublisher.js`, NEVER
...
// candidate vocabulary `application/SnapshotCandidateDiscoveryQueryService.js`
...
// role `application/SnapshotCandidateDiscoveryQueryService.js`'s own
// dedup key (`storage`+`contentHash`+`locator`), and it plays no role the
// resolver (`application/DecentralizedSnapshotResolver.js`) ever reads.
//
...
// `contentHash` stays entirely `application/DecentralizedSnapshotResolver.js`'s
...
//   `application/ArweaveSnapshotDiscoveryPublisher.js` appears anywhere in
//   this file.
// - **Querying Nostr, or any substrate besides Arweave.** No import of
//   `application/NostrSnapshotDiscoveryQueryService.js` or any Nostr module.
...
//   `application/ArweaveTaggedTransactionUpload.js`,
...
export class ArweaveSnapshotDiscoveryQueryService {
    ...
    //   `application/ArweaveSnapshotDiscoveryPublisher.js`'s own default;
...
// private helper `application/ArweaveGraphqlDiscoveryQueryService.js`
