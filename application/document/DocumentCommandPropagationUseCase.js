...
// text) found it only half true. `application/document/WorldCommandPropagationUseCase.js`
// (0.2.96-0.2.97), `application/identity/WorldMembershipUseCase.js` (0.2.98), and
...
// presence — real, shipped, wired into `application/world/CreateWorldViewUseCase.js`
// today. What is genuinely, verifiably absent — the thing 0.9.221's own
// citation of `application/document/SaveDocumentUseCase.js` was actually pointing
...
// `application/editor/EditorSession.js`), has NO propagation protocol at all:
// zero peer/collaboration imports anywhere in EditorView.js, and its only
// multi-party path is `application/editor/ForkStructureUseCase.js` — fork, then
...
//                                      already consumes
//   4. resolve Document EDIT access — application/identity/WorldAuthorizationService.js,
...
export class DocumentCommandPropagationUseCase {
    ...
    attachCommandHistory({ documentId, commandHistory }) {
        ...
        const subscription = commandHistory.eventBus.subscribe(CommandHistoryEvent.COMMAND_EXECUTED, ({ command }) => {
            ...
            // event fires (see `application/editor/CommandHistory.js#execute()`),
