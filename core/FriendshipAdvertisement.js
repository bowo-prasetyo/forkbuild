...
//                      elsewhere, at the fixed granularity a two-action,
//                      never-repeated vocabulary actually needs.
...
//                      header above.
export function toFriendshipAdvertisement({ actorIdentity, subjectIdentity, action, timestamp = Date.now() }) {
    return {
        actorIdentity,
        subjectIdentity,
        action,
...
// advertisement — actor, subject, action, sequence, AND timestamp,
// deliberately never a narrower subset such as just
...
// actually granted. `id: advertisement.actorIdentity` matches every
// other SignatureType's own "id is who this claim is ABOUT" convention;
// a verifier separately requires `signature.signer === actorIdentity`
// (see identity/LocalAuthorizationVerifier.js#verifyFriendshipAdvertisement),
