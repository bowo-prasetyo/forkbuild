...
export class World {
    ...
    // -----------------------------------------------------------------
    // World landmarks (0.3.7) — persistent, user-created points of interest.
    // Unlike StructurePlacement, landmarks are explicit World content
    // created to mark meaningful places.
    // -----------------------------------------------------------------
    ...
    updateLandmark(id, changes) {
        ...
        if (changes.title !== undefined) {
            landmark.setTitle(changes.title);
        }
        if (changes.description !== undefined) {
            landmark.setDescription(changes.description);
        }
        if (changes.position) {
            landmark.setPosition(changes.position);
        }
