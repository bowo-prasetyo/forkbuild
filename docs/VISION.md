...
- **World View** — the "Minecraft": walk through shared spaces, inspect
  bricks, and edit or place new bricks directly in the spatial
  environment.

All three are views over the same protocol data, not separate systems.

And as of 0.1.46, building in those spaces is a first-class, direct
experience: selections show an interactive transform gizmo, dragging a
handle previews the change live, releasing commits it as one undoable
operation — identically in the Editor and in the World View. The classic
editing kernel (select, transform, group, clipboard, history, replay)
is now complete under real pointer interaction; what remains is
precision (snapping, alignment) and, eventually, other people.
