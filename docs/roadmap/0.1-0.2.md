...
## Milestone list (0.1.1 to 0.2.51)
...
An advertise/pull round trip (`application/presence/PresenceSyncService.js`)
...
(`application/presence/LocalPresenceStore.js`), sequence-tolerant of exactly
...
`application/avatar/RemoteAvatarInterpolator.js`) so bursty network updates
...
`application/presence/PresenceTrustBoundary.js`, sits between "an advertisement
...
Ed25519 (`application/presence/PresenceSigning.js`,
...
(`application/avatar/AvatarProfileSyncService.js`/`AvatarProfileTrustBoundary.js`/
...
Read." `application/avatar/LocalAvatarProfileStore.js` deliberately never
...
PROPOSED position, `application/avatar/AvatarMovementConstraint.js` (backed
...
(`application/avatar/RemoteAvatarRegistry.js`), never a second,
...
`application/presence/LocalPresenceStore.js` already deletes an ABSENT record
...
`application/avatar/AvatarInteractionSyncService.js` — mirrors the shape
...
way `application/presence/PresenceSigning.js` already does, with no separate
...
record is ever created. `application/peer/DiscoverPeersUseCase.js` and
`application/peer/ConnectToPeerUseCase.js` wire the whole flow —
...
ConnectToPeerUseCase.js` and `application/peer/DiscoverPeersUseCase.js` needed
...
`application/peer/ConnectToPeerUseCase.js` gained exactly one thing:
...
shipped: `application/peer/ConnectedPeer.js#_notify()` iterated its
`_stateListeners` Set live, but `application/peer/ConnectedPeerRegistry.js`'s
...
directly on `application/peer/ConnectedPeer.js` and nothing lower:
...
built on `peer/PeerMessageBus.js` and `application/peer/ConnectedPeerRegistry.js`
...
`application/avatar/AvatarProfileSyncService.js` through `application/
...
temporary posture `application/avatar/AvatarProfileTrustBoundary.js` already
...
`application/peer/DiscoverPeersUseCase.js` and `application/
...
`application/avatar/AvatarProfileUseCase.js` already established in 0.2.33 —
...
Record." `application/identity/FriendRelationshipUseCase.js` is the first live
...
BroadcastChannel transport (`application/world/CreateWorldViewUseCase.js`,
...
succeed — see `application/identity/IdentityUseCase.js`'s call sites in
...
directory: `application/identity/CreateIdentityLifecyclePropagationUseCase.js`
...
`application/chat/ChatOutbox.js`'s own reliable-delivery model (0.2.63); and
...
├── application/chat/ConversationStore.js one flat list per LOCAL owner,
│                                    filtered by peerIdentityId, the
│                                    exact shape application/chat/ChatOutbox.js
...
│                                    the outbox
├── write-through                   application/chat/ChatUseCase.js's own
...
└── security                        ConversationStore entries are
                                     ...
                                     application/chat/ChatOutbox.js already
                                     has (0.2.63)
```

The security property this milestone cares about most is the one
`application/chat/ChatOutbox.js` already proved in 0.2.63, now extended to
...
marked DELIVERED. `application/chat/ConversationStore.js` never becomes an
authorization mechanism either: every write to it happens strictly
AFTER `application/chat/ChatUseCase.js`'s own ingestion boundary (friendship,
...
milestone's own "Never Reused As, Or By, `application/chat/ChatOutbox.js`");
an aggregate conversation-list/inbox UI —
`application/chat/ChatUseCase.js#getConversations()` now genuinely returns
...
`application/chat/ConversationStore.js#append()` is idempotent by messageId,
...
mattered: `application/chat/LiveConversation.js` was exactly as ephemeral as
...
├── application/chat/ConversationReadTracker.js  a THIRD durable per-owner
│                                          store, alongside the outbox
│                                          (0.2.63) and the history
│                                          store (0.2.69) — answers "what
│                                          have I seen," never folded
│                                          into either
├── application/presence/PeerPresenceUseCase.js   a COMPUTED reconciliation,
...
facts `application/presence/PeerPresenceUseCase.js` reconciles is allowed to
depend on any of the others: `isConnectedNow` flips to `false` the
instant `application/peer/ConnectedPeerRegistry.js` says so, while the
...
(`markRead()`, delegated to `application/chat/ConversationReadTracker.js`,
...
`application/presence/PeerPresenceUseCase.js` needed; multi-device presence
...
│   application/chat/ConversationReadOutbox.js  SENT, at most ONE per peer,
│                                          never accumulated the way
│                                          application/chat/ChatOutbox.js
...
│   application/chat/RemoteReadReceiptStore.js  of core/ConversationReadMarker.js
...
└── application/chat/ChatUseCase.js           sendReadReceipt() recomputes
...
`application/identity/FriendRelationshipUseCase.js#unfriend()` (0.2.60) already
existed, and `application/chat/ChatUseCase.js#canChat()`/`_requireEligible()`
...
QUEUED, sitting in `application/chat/ChatOutbox.js`, the instant that
...
├── application/chat/ChatOutbox.js#cancel()          discards every QUEUED
...
│                                      history CANCELLED
├── application/chat/ConversationReadOutbox.js#cancel()  the identical
│                                      operation for a still-PENDING
│                                      (never SENT) coalesced read
│                                      acknowledgement
└── application/chat/ChatUseCase.js        subscribes to
...
marker, or a peer's remote read receipt — `application/chat/ConversationStore.js`
(0.2.69), `application/chat/ConversationReadTracker.js` (0.2.70), and
`application/chat/RemoteReadReceiptStore.js` (0.2.71) already only ever
receive writes from `application/chat/ChatUseCase.js`'s own append/publish/
...
`application/chat/ConversationStore.js#MAX_STORED_MESSAGES_PER_PEER`
...
├── application/chat/LocalAudioTrackProvider.js  the one place this codebase
│                                      asks for a real microphone,
│                                      injected exactly like
│                                      storage/StorageProvider.js is
│                                      injected everywhere else
└── application/chat/VoiceUseCase.js       the orchestrator: reuses
                                       application/chat/ChatUseCase.js#canChat()'s
...
└── application/chat/VoiceUseCase.js       three additions, zero new states:
...
```text
0.2.75
├── application/chat/LocalAudioTrackProvider.js  two additions to the one
...
camera free to roam wherever `application/world/SpatialCameraController.js`
...
│                                      or deform it
└── application/world/RenderWorldViewUseCase.js  avatar ground placement: a
...
├── application/avatar/AvatarTerrainConstraint.js  the application-layer
...
└── application/world/WorldNavigationSession.js  _buildAvatarTerrainConstraint()
...
collision explicitly does — `application/avatar/AvatarTerrainConstraint.js`
...
it is not wired into `application/peer/PeerRelationshipUseCase.js`,
...
identical discipline `application/identity/IdentityLifecyclePropagationUseCase.js`
...
in isolation but is not consulted by `application/peer/PeerRelationshipUseCase.js`
(a "Known Peer" is still one record per identityId, with no device
dimension at all), `application/chat/ChatUseCase.js`/`application/chat/ChatOutbox.js`
...
`application/chat/VoiceUseCase.js` (still exactly "one call at a time, per
...
(mirroring `application/identity/IdentityLifecyclePropagationUseCase.js`'s own
...
## 0.2.82 — Multi-Device Social State Semantics
...
├── application/identity/SocialIdentityResolver.js (new)
...
├── application/identity/FriendRelationshipUseCase.js
...
├── application/chat/ChatUseCase.js
│     `_conversations`, application/chat/ChatOutbox.js, application/
│     ConversationStore.js, application/chat/ConversationReadOutbox.js,
│     application/chat/RemoteReadReceiptStore.js, and canChat()/_requireEligible()
...
└── application/chat/VoiceUseCase.js
...
not one authentication check in application/identity/FriendRelationshipUseCase.js/
application/chat/ChatUseCase.js/application/chat/VoiceUseCase.js changed shape; a
...
named directly: `application/identity/FriendRelationshipUseCase.js#acceptFriendRequest()`
...
the palette, mirroring `application/editor/EditorActionRegistry.js#groupByCategory()`'s
...
motivation `application/editor/EditorActionRegistry.js#groupByCategory()`
...
`core/AvatarCollision.js` and `application/editor/SelectionBoundsService.js`
...
└── application/editor/ForkStructureUseCase.js  Structure -> new Document,
...
what `application/publication/ForkPublishedWorldUseCase.js` already does for a whole
...
Alice's Laptop are two completely independent application/chat/ChatUseCase.js
...
│   application/chat/SiblingReadStateStore.js        "what has each of my OWN
...
└── application/chat/DeviceConversationSyncUseCase.js (new)
...
read straight from application/identity/DeviceAuthorizationPropagationUseCase.js's
...
message advances ONLY Laptop's own application/chat/ConversationReadTracker.js
...
application/chat/ConversationStore.js's own dedup, never suppressed by
...
│                                        since 0.2.80
├── application/editor/LibraryPreviewService.js   a smaller sibling of
...
├── application/editor/CreateLibraryPreviewUseCase.js  constructs one per
│                                        Editor session
...
identity instead of its raw key. `application/presence/PeerPresenceUseCase.js`
...
```text
0.2.85
└── application/presence/PeerPresenceUseCase.js   gains an optional
...
counterpart to `application/chat/ChatUseCase.js#_findAuthenticatedPeer()`,
...
`application/peer/ConnectedPeerRegistry.js` on every call; no device-level
...
```text
0.2.86
├── application/peer/ConnectedIdentityPeers.js   NEW — findLiveConnectedPeers(),
│                                            application/presence/PeerPresenceUseCase.js's
│                                            own 0.2.85 `_liveConnectedPeers()`
│                                            filter, extracted verbatim so
│                                            VoiceUseCase can reuse the
│                                            IDENTICAL query rather than
│                                            re-deriving it
├── application/presence/PeerPresenceUseCase.js      _liveConnectedPeers() becomes a
│                                            thin call into the shared
│                                            function above — behavior
│                                            byte-identical to 0.2.85
└── application/chat/VoiceUseCase.js             startCallToIdentity(identityId),
...
`application/presence/PeerPresenceUseCase.js#_liveConnectedPeers()` already was
...
│                                                 now also computes
│                                                 collision validity
├── application/editor/PreviewUseCase.js               show() gains a 4th
│                                                 `valid` argument
...
│                                                 pre-existing `valid`
├── application/world/WorldNavigationSession.js       rotatePlacementPreview(),
...
├── application/world/RenderWorldViewUseCase.js       showPreview() threads the
│                                                 new `valid` argument
...
nothing more: `application/world/WorldNavigationSession.js#_presentPlacementPreview()`
now calls `core/TerrainHeightField.js#terrainHeightAt()` directly (never
through `renderer.terrainHeightAt()` — the same discipline
`application/avatar/AvatarTerrainConstraint.js` established in 0.2.77), sampled
...
## 0.2.88 — Deterministic World Ecology
...
`application/avatar/AvatarTerrainConstraint.js` (walkability stays exactly the
...
`application/avatar/AvatarTerrainConstraint.js` stay exactly the slope-only
...
│                                                 whole in 0.2.90)
├── application/editor/StructureDocumentResolver.js     NEW — documentId -> World,
│                                                 resolved FRESH from
│                                                 storage every call, null
│                                                 (never throws) when
│                                                 unresolvable
├── application/editor/StructurePlacementValidator.js   NEW — conservative,
│                                                 translate-only AABB
...
│                                                 rung up
├── application/editor/CreateCommandRegistryUseCase.js registers 'place-structure'/
...
│                                                 one rung up
├── application/editor/StructurePreviewUseCase.js      NEW — show()/hide(), mirrors
│                                                 PreviewUseCase
...
├── application/editor/EditorContext.js                + activeStructure/
...
│                                                 ground-snap math
├── application/editor/CreateToolRegistryUseCase.js    registers PLACE_STRUCTURE
├── application/editor/EditorSession.js                + structureResolver/
│                                                 structurePreviewUseCase
│                                                 deps, placeDocument(),
│                                                 removeStructurePlacement()
├── application/editor/EditorActionContext.js          placementMode widened to
│                                                 cover PLACE_STRUCTURE too
├── application/document/CreatePersistenceUseCase.js     + structureDocumentResolver
├── application/world/RenderWorldUseCase.js           + structureResolver and
│   application/world/RenderWorldViewUseCase.js         TransformMath threaded
│                                                 into WorldRenderer
...
time. `application/editor/StructureDocumentResolver.js` is the whole mechanism
...
`application/editor/TransformMath.js#rotatePointAroundPivotY()` (injected, the
same way `application/world/RenderWorldUseCase.js` already injects it into
...
`application/editor/StructurePlacementValidator.js` computes the candidate
...
`application/editor/EditorSession.js#placeDocument()` refuses to target the
...
(`application/world/RenderWorldViewUseCase.js` threads `structureResolver`
...
│                                                 selectedPlacementId
├── application/editor/SelectionUseCase.js             + selectPlacement()
...
├── application/editor/CreateCommandRegistryUseCase.js registers all three
...
│                                                 dragging an
│                                                 already-placed one
├── application/editor/InputDispatcher.js              + pickedPlacement (a
│                                                 second, optional,
│                                                 always-disjoint pick)
├── application/world/RenderWorldUseCase.js           wires placementMeshRegistry
│                                                 into PickingService/
...
│                                                 headless, no 'three'
│                                                 import
├── application/editor/EditorSession.js                moveSelection()/
│                                                 rotateSelection()/
...
├── application/editor/EditorActionContext.js          + selectionIsStructurePlacement
├── application/editor/EditorActionRegistry.js         + 'selection.duplicate'
│                                                 (Ctrl/Cmd+D); existing
│                                                 nudge/rotate/delete
...
`application/editor/EditorSession.js#moveSelection()`/`rotateSelection()`/
...
brickId. `application/editor/InputDispatcher.js` computes `pickedPlacement`
...
```text
0.2.92
├── application/editor/StructurePlacementGestureService.js  NEW — the
│                                                 StructurePlacement
│                                                 counterpart to
...
│                                                 target (see
│                                                 docs/Principles.md).
├── application/editor/GizmoGestureRouter.js            NEW — the ONE new piece
...
│                                                 than { pivot, bounds }.
├── application/editor/EditorSession.js                 + a second gesture
...
- application/editor/EditorActionRegistry.js — one registry of user-facing
...
- application/editor/EditorActionContext.js — pure snapshot of availability
...
- application/editor/InputRouter.js — minimal input routing: the explicit
...
│                                                 brickId
├── application/editor/SpatialInspectionService.js      + _inspectPlacement() —
...
│                                                 local x/z.
├── application/world/WorldNavigationSession.js       + structureResolver/
│                                                 loadDocumentUseCase
│                                                 (constructor, both
│                                                 optional); + pick() now
...
│                                                 once.
├── application/world/RenderWorldViewUseCase.js       PickingService now built
...
├── application/world/CreateWorldViewUseCase.js       + wires a real
...
discipline. `application/editor/SelectionBoundsService.js#calculate()` iterates
...
check added to either file. `application/editor/SpatialEditingService.js#
...
through `application/editor/CameraFocusAnimator.js`, a pure interpolation
...
│                                                 EDITOR/ADMIN/
│                                                 MODERATOR anywhere.
├── application/identity/WorldAuthorizationService.js      NEW — resolveAccess(document)
│                                                 -> WorldAccessLevel,
│                                                 + canRead()/canEdit().
...
│                                                 parentStructureId
│                                                 (0.2.81) already set.
├── application/document/DocumentCloneService.js,          + authorIdentityId,
...
│                                                 is never inherited,
│                                                 only content is.
├── application/editor/SpatialEditingService.js          + a `canEditDocument`
│                                                 predicate (5th ctor
│                                                 arg, default `() =>
│                                                 true` — every existing
...
├── application/world/WorldNavigationSession.js         + worldAuthorizationService
│                                                 (optional ctor
│                                                 collaborator, same
│                                                 graceful-absence
...
│                                                 at construction.
├── application/world/CreateWorldViewUseCase.js         + builds ONE
│                                                 WorldAuthorizationService,
...
predicate lives in `application/editor/SpatialEditingService.js`, not in
`application/world/WorldNavigationSession.js`. Every mutation method
...
what `application/editor/EditorSession.js` builds its own gesture service from
...
│                                                 snapshot.
├── application/document/WorldCommandPropagationUseCase.js NEW — the protocol,
│                                                 riding its own
...
│                                                 "what a mutation is."
└── application/identity/WorldAuthorizationService.js      REUSED, unmodified — the
...
`application/chat/ChatUseCase.js#_handleIncoming()`'s own ordered gate one rung
further:
...
`application/chat/DeviceConversationSyncUseCase.js` was in 0.2.83 — but this
milestone does not thread `broadcastCommand()`/`onOperationApplied()`
into `application/world/WorldNavigationSession.js`'s own per-World
...
plain subscription to `application/editor/CommandHistory.js`'s own existing
`COMMAND_EXECUTED` event, the SAME event every other consumer already
uses. `application/world/CreateWorldViewUseCase.js` now builds a real
...
│                                                 be re-granted.
├── application/identity/WorldMembershipUseCase.js         NEW — grantEdit()/
│                                                 revokeEdit()/
│                                                 hasActiveGrant()/
...
│                                                 May Ever Issue A
│                                                 Membership Grant."
├── application/identity/WorldAuthorizationService.js      MODIFIED — resolveAccess()/
│                                                 canEdit()/canRead() now
│                                                 accept an OPTIONAL
...
│                                                 purpose.
├── application/document/WorldCommandPropagationUseCase.js MODIFIED — its own
│                                                 receiving-side
│                                                 WorldAuthorizationService
│                                                 now threads
│                                                 `resolveWorldEditGrant`
...
│                                                 from a claim on the
│                                                 wire.
└── application/presence/WorldPresenceUseCase.js           NEW — enterWorld()/
                                                  setActivity()/
                                                  leaveWorld()/
...
`application/world/WorldNavigationSession.js` gains a thin, purely delegating
...
`application/world/CreateWorldViewUseCase.js` wires both, peer-stack-gated
...
│                                                  surface, two entry
│                                                  points.
├── application/world/WorldNavigationSession.js         + `authorIdentityId`
