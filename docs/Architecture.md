...
| `scripts/steem-threads/` | Operator pages, not part of the app: one creates the monthly Steem discovery threads, and `content-check.html` checks that API nodes return content stored on Steem in full | `docs/Protocol.md`, "Proposed: Steem Announcement Substrate" |
