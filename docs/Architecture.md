...
| `steem/` | The Steem JSON-RPC client (API node failover) and the Steem Keychain broadcaster. The app reads Steem through `application/steem/` (a discovery thread reader and one adapter per announcement family, composed in `ui/main/composeWorldDiscovery.js`); the Keychain broadcaster is used so far only by `scripts/steem-threads/` | `docs/Protocol.md`, "Proposed: Steem Announcement Substrate" |
