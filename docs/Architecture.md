...
| `steem/` | The Steem JSON-RPC client (API node failover) and the Steem Keychain broadcaster. The app reads and announces on Steem through `application/steem/` (a discovery thread reader, an announcer, and a reader and publisher per announcement family, composed by `SteemRuntimeComposition.js` in `ui/main/composeWorldDiscovery.js`); `scripts/steem-threads/` uses the same client and broadcaster to create the threads | `docs/Protocol.md`, "Proposed: Steem Announcement Substrate" |
