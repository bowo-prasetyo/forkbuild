...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        ...
        const moduleSource = await (await import('node:fs/promises')).readFile(new URL('../application/claimSnapshotReconciliation/decision/HistoryStatisticsView.js', import.meta.url), 'utf8');
        ...
        assert(importLines.length === 1, '63. besides shared helpers, this file imports exactly one module');
        assert(importLines[0].includes('./HistoryView.js'), '64. the one import is the 0.8.150 archive reconstruction seam, never the reconciliation family itself');
    }
...
run().catch((error) => {
    console.error('ReconciliationDecisionHistoryStatisticsView.test.js FAILED:', error);
    process.exitCode = 1;
});
