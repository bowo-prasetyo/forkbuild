...
} from '../application/claimSnapshotReconciliation/decision/HistorySynchronization.js';
...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        ...
        const moduleSource = await (await import('node:fs/promises')).readFile(new URL('../application/claimSnapshotReconciliation/decision/HistorySynchronization.js', import.meta.url), 'utf8');
        ...
        assert(importedFiles.some((path) => path === './HistoryDifference.js') && importedFiles.some((path) => path === './HistoryExchange.js'), '66. the two imports are exactly the 0.8.149 difference projection and the 0.8.151 exchange — never the plan/candidate/decision/archive modules directly');
    }
