...
import { describePublisherLeaderboardClaimSnapshotReconciliationDecisionHistoryDifference } from '../application/claimSnapshotReconciliation/decision/HistoryDifference.js';
import {
...
} from '../application/claimSnapshotReconciliation/decision/HistoryExchange.js';
...
// The identical shared world `ReconciliationDecisionHistoryDifference.test.js`
...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        const moduleSource = await (await import('node:fs/promises')).readFile(new URL('../application/claimSnapshotReconciliation/decision/HistoryExchange.js', import.meta.url), 'utf8');
        ...
        assert(importLines.length === 1, '72. besides shared helpers, this file imports exactly one module');
        assert(importLines[0].includes('./History.js') && !importLines[0].includes('Difference') && !importLines[0].includes('View'), '73. the one import is 0.8.146\'s own append boundary, never the plan/candidate/decision/difference/archive modules');
