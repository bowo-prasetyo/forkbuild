...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        const moduleSource = await (await import('node:fs/promises')).readFile(new URL('../application/claimSnapshotReconciliation/decision/Decision.js', import.meta.url), 'utf8');
        const importLines = moduleSource.split('\n').filter((line) => line.startsWith('import '));
        assert(importLines.length === 1 && importLines[0].includes("'../ReconciliationCandidate.js'"), '19. this file imports exactly one thing — 0.8.144\'s own selection boundary, nothing else');
