...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        ...
        // see application/publication/distribution/PublicationDistributionRuntimeProvider.js's own
    ...
    // ---------------------------------------------------------------
    {
        ...
        assert(codeOnly.includes('createPublicationDistributionRuntimeProvider({') && codeOnly.includes('...nostrPublicationRuntimeCapabilities'),
            '16. ui/main.js actually calls the new provider factory, now fed through the 0.9.108 Nostr runtime adapter — still with nothing real to supply yet, exactly today\'s honest state');
