...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        const moduleSource = await (await import('node:fs/promises')).readFile(
            new URL('../application/claimSnapshotReconciliation/leaderboard/EvidenceExport.js', import.meta.url), 'utf8'
        );
        ...
        assert(codeOnly.includes("from '../candidate/FilteredEvidenceDetailView.js'"), '29. one import is 0.8.185\'s own filtered evidence detail module');
        assert(codeOnly.includes("from './EvidenceFilter.js'"), '30. another import is 0.8.184\'s own evidence filter module (its enums)');
        assert(codeOnly.includes("from './ComparisonState.js'"), '31. another import is 0.8.183\'s own comparison-state module');
