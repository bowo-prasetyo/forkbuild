...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        const moduleSource = await (await import('node:fs/promises')).readFile(new URL('../application/claimSnapshotReconciliation/leaderboard/LeaderboardReadModel.js', import.meta.url), 'utf8');
        ...
        assert(importBlock.includes('../candidate/EvidenceAgreementView.js'), '49. that one import is 0.8.176\'s own candidate evidence agreement projection');
