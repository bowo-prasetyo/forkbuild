...
async function run() {
    ...
    // ===============================================================
    {
        ...
        check(publishAttemptedOn === 'nostr', 'I1. despite discoveryProvider: "arweave" being passed to the real, production orchestratePublicationDistribution(), the actual publish attempt happened on NOSTR — this function\'s own parameter list never forwards discoveryProvider or arweaveAnnouncementPublisherOptions to composePublicationDistributionRuntime(), so both are silently dropped by ordinary object destructuring and the "nostr" default always wins');
        check(result.discovery.relayUrl === NostrPublicationDiscoveryPublisher.DEFAULT_RELAY_URL, 'I2. ...confirmed structurally too: the resulting discovery.relayUrl is Nostr\'s own default wss:// relay, never this call\'s intended Arweave gateway');

        // Source sweep: confirm the same gap holds through every real
        // layer between the orchestrator and ui/main.js itself — not just
        // the one orchestrator entry point this section already exercised
        // live.
        const chainFiles = [
        ...
        for (const file of chainFiles) {
            ...
            check(!/discoveryProvider|arweaveAnnouncementPublisherOptions/.test(code), `I3[${file}]. no reference anywhere in this file to discoveryProvider or arweaveAnnouncementPublisherOptions — the selector composePublicationDistributionRuntime() itself accepts (application/PublicationDistributionRuntimeComposition.js, the ONLY file in this codebase that mentions it in production source) never gets threaded this far up the real call chain`);
        }
        ...
        console.log('✓ Section I: UI_SELECTION_MECHANISM_GAP — discoveryProvider: "arweave" is real, tested, and completely unreachable through the real production call chain from ui/main.js down through the orchestrator; every real caller silently gets "nostr", confirmed both live (the orchestrator itself drops the field) and by source sweep of every intermediate layer');
    }
    ...
    // ===============================================================
    {
        const VERDICT = Object.freeze({
            ...
            nostrIsolation: 'PASS',
            uiReachability: 'UI_SELECTION_MECHANISM_GAP',
        ...
        check(VERDICT.uiReachability === 'UI_SELECTION_MECHANISM_GAP', 'K2. the one incomplete boundary is reachability from a real caller, never correctness of what would run if reached');
        check(assertionCount > 40, 'K3. sanity: this audit is substantive, not a token pass');

        console.log('\nVerdict: ARWEAVE_DISCOVERY_PATH_COMPLETE_UI_GAP');
        for (const [key, value] of Object.entries(VERDICT)) {
            console.log(`  ${key.padEnd(26)} ${value}`);
        }
        console.log(`\n(${assertionCount} checks)`);
        console.log('\nThe precise seam a future milestone would need to close this gap (named here, not built — this audit is test-only):');
        console.log('  application/PublicationDistributionOrchestrator.js        — forward discoveryProvider + arweaveAnnouncementPublisherOptions to composePublicationDistributionRuntime()');
        console.log('  application/PublicationDistributionCommand.js             — forward both through to the orchestrator call');
        console.log('  application/PublicationDistributionCommandComposition.js  — bind both alongside lifecycleStore');
        console.log('  application/PublicationDistributionConfigurationProvider.js — a resolveArweaveAnnouncementPublisherOptions() resolver, mirroring resolveNostrPublisherOptions()');
        console.log('  application/PublicationDistributionRuntimeConfiguration.js  — thread an announcement-options section through');
        console.log('  ui/main.js                                                 — supply a real discoveryProvider choice and arweaveAnnouncementPublisherOptions at the composition root');
        console.log('\nPer this milestone\'s own recommendation: since the gap is UI_SELECTION_MECHANISM_GAP rather than a reader or correctness gap, the natural next step is the smallest contextual selection mechanism above — never multi-substrate fan-out, a generic SubstrateProvider, or a new discovery semantic.');
    }
