...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        const moduleSource = await (await import('node:fs/promises')).readFile(
            new URL('../application/claimSnapshotReconciliation/leaderboard/EvidenceImport.js', import.meta.url), 'utf8'
        );
        ...
        assert(codeOnly.includes("from './EvidenceExport.js'"), '29. one import is 0.8.186\'s own evidence-export module (its protocol-version constant)');
