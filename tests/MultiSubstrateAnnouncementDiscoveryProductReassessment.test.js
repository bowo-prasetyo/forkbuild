...
async function run() {
    ...
    // ===============================================================
    {
        ...
        // own header. Its own :disabled expression is unmodified.
        ...
        // AMENDED — One Shared Distribution Settings Block: the shared
        // select is disabled while EITHER protocol is mid-call.
        assert(/:disabled="anyExecuting"[^>]*>\s*<option value="arweave">/.test(dialogSource.replace(/\n\s*/g, ' '))
            && /anyExecuting\(\) \{\s*return this\.distributionExecuting \|\| this\.snapshotDistributionExecuting;/.test(dialogSource), n('H2. the substrate <select> is disabled only mid-call, never after a completed distribution — a Wanderer can immediately reselect the other substrate'));
