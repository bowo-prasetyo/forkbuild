...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        ...
        const moduleSource = await (await import('node:fs/promises')).readFile(new URL('../application/claimSnapshotReconciliation/candidate/EvidenceDetailView.js', import.meta.url), 'utf8');
        ...
        assert(importBlock.includes('./EvidenceAgreementView.js'), '72. the one import is 0.8.176\'s own candidate evidence agreement projection');
