...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        ...
        const moduleSource = await (await import('node:fs/promises')).readFile(new URL('../application/claimSnapshotReconciliation/revalidationObservation/HistorySynchronization.js', import.meta.url), 'utf8');
        ...
        assert(importedFiles.some((path) => path === './HistoryDifference.js') && importedFiles.some((path) => path === './HistoryExchange.js'), '76. the two imports are exactly the 0.8.166 difference projection and the 0.8.168 exchange — never the observation/history/deduplication/timeline/archive modules directly');
    }
