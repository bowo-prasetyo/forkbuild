...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        ...
        const module = await import('../application/claimSnapshotReconciliation/revalidationObservation/History.js');
...
run().catch((error) => {
    console.error('ReconciliationDecisionRevalidationObservationHistory.test.js FAILED:', error);
    process.exitCode = 1;
});
