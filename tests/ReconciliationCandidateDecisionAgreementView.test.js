...
} from '../application/claimSnapshotReconciliation/candidate/DecisionAgreementView.js';
...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        ...
        const moduleSource = await (await import('node:fs/promises')).readFile(new URL('../application/claimSnapshotReconciliation/candidate/DecisionAgreementView.js', import.meta.url), 'utf8');
        ...
        assert(importBlock.includes('../decision/HistoryView.js'), '81. one import is 0.8.150\'s own archive-reading seam');
