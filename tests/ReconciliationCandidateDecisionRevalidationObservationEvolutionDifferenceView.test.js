...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        ...
        const moduleSource = await (await import('node:fs/promises')).readFile(new URL('../application/claimSnapshotReconciliation/candidate/DecisionRevalidationObservationEvolutionDifferenceView.js', import.meta.url), 'utf8');
        ...
        assert(importBlock.includes('./DecisionRevalidationObservationEvolutionView.js'), '80. the other import is 0.8.172\'s own candidate observation evolution module');
