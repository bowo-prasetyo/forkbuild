...
} from '../application/claimSnapshotReconciliation/decision/CandidateCorrespondenceView.js';
...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        ...
        const moduleSource = await (await import('node:fs/promises')).readFile(new URL('../application/claimSnapshotReconciliation/decision/CandidateCorrespondenceView.js', import.meta.url), 'utf8');
...
run().catch((error) => {
    console.error('ReconciliationDecisionCandidateCorrespondenceView.test.js FAILED:', error);
    process.exitCode = 1;
});
