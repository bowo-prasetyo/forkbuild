...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        ...
        const moduleSource = await fs.readFile(new URL('../application/claimSnapshotReconciliation/decision/HistoryRevalidationPlanIdentityView.js', import.meta.url), 'utf8');
        ...
        assert(importLines.some((line) => line.includes('./HistoryRevalidationView.js')), '77. one import is 0.8.158\'s own decision history revalidation projection');
        assert(importLines.some((line) => line.includes('../PlanIdentity.js')), '78. the other import is 0.8.160\'s own plan identity projection');
        ...
        const module = await import('../application/claimSnapshotReconciliation/decision/HistoryRevalidationPlanIdentityView.js');
