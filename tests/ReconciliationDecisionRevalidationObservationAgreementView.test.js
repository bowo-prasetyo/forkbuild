...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        ...
        const moduleSource = await (await import('node:fs/promises')).readFile(new URL('../application/claimSnapshotReconciliation/revalidationObservation/AgreementView.js', import.meta.url), 'utf8');
        ...
        const module = await import('../application/claimSnapshotReconciliation/revalidationObservation/AgreementView.js');
...
run().catch((error) => {
    console.error('ReconciliationDecisionRevalidationObservationAgreementView.test.js FAILED:', error);
    process.exitCode = 1;
});
