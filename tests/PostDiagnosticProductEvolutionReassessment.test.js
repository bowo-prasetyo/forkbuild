...
async function runTests() {
    ...
    const inventory = [];
    {
        ...
        // 0.9.323's own A-nav) as an EXACT route set — unaffected by
        // either 0.9.324 or 0.9.325, since Diagnostic Tools lives entirely
        // inside World View's own OwnPublicationPanel, never as a new
        // top-level nav destination. 0.9.393 — this file's own count had
        // gone stale: the eleven-route set below predates 0.9.364-0.9.372
        // (Arweave Gateway/Nostr Relay settings) and 0.9.386/0.9.388
        // (STUN/Rendezvous settings), each of which added a real,
        // always-mounted nav route with no relation to Diagnostic Tools,
        // and nothing re-ran this guard to notice until 0.9.393's own
        // full-suite execution. Extended to fifteen now, per this
        // milestone's own classification, never a bare count bump.
