...
async function run() {
    ...
    // for 0.9.112: exactly seven application/ modules now, still no core/).
    // ---------------------------------------------------------------
    {
        const source = await readFile(new URL('../ui/components/WorldEncounterCanvas.js', import.meta.url), 'utf8');
        const importLines = source.split('\n').filter((line) => line.trim().startsWith('import '));
        assert(importLines.length === 9, '37. WorldEncounterCanvas.js has exactly nine imports as of 0.9.112');
        ...
        assert(applicationImportLines.length === 7, '40. WorldEncounterCanvas.js imports exactly seven application/ modules as of 0.9.112');
