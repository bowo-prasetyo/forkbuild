...
async function run() {
    ...
    // for 0.9.100: exactly six application/ modules now, still no core/).
    // ---------------------------------------------------------------
    {
        const source = await readFile(new URL('../ui/components/WorldEncounterCanvas.js', import.meta.url), 'utf8');
        const importLines = source.split('\n').filter((line) => line.trim().startsWith('import '));
        assert(importLines.length === 8, '37. WorldEncounterCanvas.js has exactly eight imports as of 0.9.100');
        ...
        assert(applicationImportLines.length === 6, '40. WorldEncounterCanvas.js imports exactly six application/ modules as of 0.9.100');
