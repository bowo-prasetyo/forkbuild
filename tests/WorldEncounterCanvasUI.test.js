...
//
// 0.9.182 note: WorldEncounterCanvas.js gained `armedForComparisonSelection`/
// `comparisonEncounter`/`comparisonSelectionOutcome` page-local state, five
// new computeds (`comparisonResolvedSelection`, `comparisonEncounterInspection`,
// `comparisonEncounterPresentation`, `selectedPublicationComparisonCandidate`,
// `comparisonPublicationComparisonCandidate`, `worldSnapshotComparisonResult`),
// four new methods (`armComparisonSelection()`/`selectComparisonEncounter()`/
// `refreshComparisonSelectionOutcome()`/`clearComparisonSelection()`), and two
// new application/ imports — see Section J, updated again below. Explicitly
// comparing two selected World Publications is covered separately, in
// tests/WorldSnapshotComparisonUI.test.js; this file's own sections stay
// focused on 0.9.3/0.9.4's own projection/selection contract, unaffected by
// that addition.
...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        ...
        const importLines = source.split('\n').filter((line) => line.trim().startsWith('import '));
        assert(importLines.length === 16, '37. WorldEncounterCanvas.js has exactly sixteen imports as of 0.9.183');
        ...
        assert(applicationImportLines.length === 14, '40. WorldEncounterCanvas.js imports exactly fourteen application/ modules as of 0.9.183');
        ...
        assert(applicationImportLines.some((line) => line.includes('WorldEncounterPresentation.js') && line.includes('describeWorldEncounterPresentation')), '48c. 0.9.176 adds exactly one new application/ import — WorldEncounterPresentation.js\'s own describeWorldEncounterPresentation(), a pure join over selectedEncounterInspection/resolvedEncounterSelection, both already computed');
        assert(applicationImportLines.some((line) => line.includes('WorldSnapshotInspection.js') && line.includes('describeWorldSnapshotInspection')), '48d. 0.9.177 adds exactly one new application/ import — WorldSnapshotInspection.js\'s own describeWorldSnapshotInspection(), a pure join over selectedEncounterPresentation/resolvedEncounterSelection, both already computed');
        assert(applicationImportLines.some((line) => line.includes('MaterializedSnapshotWorldDiscoveryBridge.js') && line.includes('unregisterMaterializedSnapshotWorldSource') && !line.includes('registerMaterializedSnapshotWorldSource(')), '48e. 0.9.179 adds exactly one new application/ import — MaterializedSnapshotWorldDiscoveryBridge.js\'s own unregisterMaterializedSnapshotWorldSource() ALONE, never its registerMaterializedSnapshotWorldSource() counterpart, which stays OwnPublicationPanel.js\'s own exclusive caller');
        assert(applicationImportLines.some((line) => line.includes('WorldEncounterComparisonCandidate.js') && line.includes('describeWorldEncounterComparisonCandidate')), '48f. 0.9.182 adds one new application/ import — WorldEncounterComparisonCandidate.js\'s own describeWorldEncounterComparisonCandidate(), a pure, source-family-agnostic join over presentation/resolvedSelection, both already computed');
        assert(applicationImportLines.some((line) => line.includes('WorldSnapshotComparison.js') && line.includes('compareSnapshotWorldPublications')), '48g. 0.9.182 adds the second of its two new application/ imports — WorldSnapshotComparison.js\'s own compareSnapshotWorldPublications() (0.9.181, unmodified)');
