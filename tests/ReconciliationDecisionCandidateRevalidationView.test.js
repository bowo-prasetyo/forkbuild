...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        ...
        const moduleSource = await fs.readFile(new URL('../application/claimSnapshotReconciliation/decision/CandidateRevalidationView.js', import.meta.url), 'utf8');
        ...
        // file's own header.
        const revalidationModule = await import('../application/claimSnapshotReconciliation/decision/CandidateRevalidationView.js');
...
run().catch((error) => {
    console.error('ReconciliationDecisionCandidateRevalidationView.test.js FAILED:', error);
    process.exitCode = 1;
});
