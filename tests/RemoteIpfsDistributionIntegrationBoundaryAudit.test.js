...
async function run() {
    ...
    // =======================================================================
    {
        ...
        const mainSource = await source('ui/main.js');
        assert(/new CreateSnapshotPlacementResolutionCoordinatorUseCase\(\)\.execute\(\{[\s\S]*?stores:\s*\[publicationContentStore,\s*new IpfsGatewayContentStore\(\)\]/.test(mainSource),
            n('E4. ui/main.js already keeps a SECOND, independent SnapshotPlacementStoreRegistry for RESOLUTION (backed by content/IpfsGatewayContentStore.js for \'ipfs\'), entirely separate from the CREATION registry Section A/E1-E3 exercised — confirmed against current source, and explicitly justified there as "never silently overwrites or hides Kubo."'));
    }
