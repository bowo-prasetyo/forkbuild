...
async function run() {
    ...
    // ===============================================================
    {
        ...
        // IceServerConfiguration.js (+ storage/IceServerConfigurationStore.js,
        // application/settings/SetIceServerConfigurationUseCase.js,
    ...
    // ===============================================================
    {
        ...
        try {
            const statusOutput = execSync('git status --porcelain -- . ":(exclude)ui/components/PublicationCard.js" ":(exclude)ui/components/PublicationList.js" ":(exclude)core/BitcoinEsploraConfiguration.js" ":(exclude)storage/BitcoinEsploraConfigurationStore.js" ":(exclude)application/settings/SetBitcoinEsploraConfigurationUseCase.js" ":(exclude)ui/views/BitcoinEsploraSettingsView.js" ":(exclude)ui/router/index.js" ":(exclude)ui/views/NetworkSettingsView.js" ":(exclude)ui/main.js" ":(exclude)css/main.css" ":(exclude)ui/views/EditorView.js"' /* AMENDED BY 0.9.638 -- excludes ui/components/PublicationCard.js/PublicationList.js, its own unrelated, separately-justified Commentary distribution-selector UI change. AMENDED FURTHER -- excludes the Bitcoin Endpoint Settings UI files: this audit's own DEFER for Bitcoin Esplora specifically was reopened on a concrete new requirement — see tests/BitcoinEndpointConfigurationUIReachabilityAudit.test.js's own reopened header. AMENDED FURTHER -- excludes ui/views/EditorView.js: its own, separately-justified Snapshot Distribution action (mirroring WorldView.js's own distributeWorldEncounterSnapshot()), unrelated to infrastructure endpoint configuration of any kind. */, { cwd: SOURCE_ROOT.pathname }).toString();
