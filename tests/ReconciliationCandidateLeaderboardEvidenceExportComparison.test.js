...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        const moduleSource = await (await import('node:fs/promises')).readFile(
            new URL('../application/claimSnapshotReconciliation/evidenceExportComparison/EvidenceExportComparison.js', import.meta.url), 'utf8'
        );
        ...
        assert(codeOnly.includes("from '../leaderboard/EvidenceFilter.js'"), '49. one import is 0.8.184\'s own evidence filter module (its enums)');
        assert(codeOnly.includes("from '../leaderboard/ComparisonState.js'"), '50. the other import is 0.8.183\'s own comparison-state module');
...
run().catch((error) => {
    console.error('ReconciliationCandidateLeaderboardEvidenceExportComparison.test.js FAILED:', error);
    process.exitCode = 1;
});
