...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        const moduleSource = await (await import('node:fs/promises')).readFile(
            new URL('../application/claimSnapshotReconciliation/evidenceExportComparison/RecordDifferenceView.js', import.meta.url), 'utf8'
        );
...
run().catch((error) => {
    console.error('ReconciliationCandidateLeaderboardEvidenceExportComparisonRecordDifferenceView.test.js FAILED:', error);
    process.exitCode = 1;
});
