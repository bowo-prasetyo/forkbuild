...
async function run() {
    ...
    // ---------------------------------------------------------------
    {
        const moduleSource = await (await import('node:fs/promises')).readFile(
            new URL('../application/claimSnapshotReconciliation/candidate/FilteredEvidenceDetailView.js', import.meta.url), 'utf8'
        );
        ...
        assert(codeOnly.includes("from './EvidenceDetailView.js'"), '34. one import is 0.8.182\'s own evidence detail view');
        assert(codeOnly.includes("from '../leaderboard/EvidenceFilter.js'"), '35. the other import is 0.8.184\'s own evidence filter module (its enums)');
    ...
    // ---------------------------------------------------------------
    {
        ...
        assert(codeOnly.includes("from '../../application/claimSnapshotReconciliation/candidate/FilteredEvidenceDetailView.js'"), '41. the view imports 0.8.185\'s own filtered-evidence-detail module');
